...
describe JavaBuildpack::Container::Tcserver do
  ...
  let(:configuration) do
    { 'tcserver'            => tcserver_configuration,
  ...
  let(:tcserver_configuration) { double('tcserver-configuration') }
  ...
  it 'should create submodules' do
    ...
    .to receive(:new).with(sub_configuration_context(tcserver_configuration))
...
#    expect(JavaBuildpack::Container::TomcatInsightSupport).to receive(:new).with(context)
  ...
  it 'should return command' do
    ...
    expect(component.command).to eq("#{java_home.as_env_var} JAVA_OPTS=\"test-opt-2 test-opt-1 -Dhttp.port=$PORT\" " \
                                      '$PWD/.java-buildpack/tcserver/tcruntime-ctl.sh start tcserver')
  end
