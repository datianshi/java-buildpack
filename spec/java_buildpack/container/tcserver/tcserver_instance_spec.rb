...
describe JavaBuildpack::Container::TcserverInstance do
  ...
  let(:component_id) { 'tcserver' }

  it 'should always detect' do
    expect(component.detect).to eq("tcserver-instance=#{version}")
  end
  ...
  it 'should extract tcserver from a GZipped TAR',
     show_output: true,
     app_fixture:   'container_tomcat',
     cache_fixture: 'stub-tcserver.tar.gz' do

    component.compile

    expect(sandbox + 'tcruntime-ctl.sh').to exist
    expect(sandbox + 'tcserver').to exist
    expect(sandbox + 'tcserver/conf/context.xml').to exist
    expect(sandbox + 'tcserver/conf/server.xml').to exist
  end
  ...
  it 'should link only the application files and directories to the ROOT webapp',
    ...
    root_webapp = app_dir + '.java-buildpack/tcserver/tcserver/webapps/ROOT'
